...
export const VitaPressPromo: React.FC = () => {
  const { fps } = useVideoConfig();

  const TRANSITION_FRAMES = Math.round(1 * fps); // 1 second crossfade
  const INTRO_DURATION = 5 * fps; // 5 seconds
  const BOTTLE_HERO_DURATION = 4 * fps; // 4 seconds
  const INGREDIENT_DURATION = 2.5 * fps; // 2.5 seconds per ingredient
  const TEXT_CARD_DURATION = 6 * fps; // 6 seconds — more time to read
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: theme.bg }}>
      <TransitionSeries>
        ...
        {/* === DETOX: Bottle Hero === */}
        <TransitionSeries.Sequence durationInFrames={BOTTLE_HERO_DURATION}>
          <BottleHero videoFile="detox-bottle.mp4" trimStartSeconds={1} />
        </TransitionSeries.Sequence>
